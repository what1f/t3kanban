...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(DESKTOP_STAGE_PACKAGE_NAME, "t3kanban");
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Kanban (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3 Kanban (Nightly)");
  });
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.equal(mac.productName, "T3 Kanban (Alpha)");
      assert.equal(mac.artifactName, "T3-Kanban-${version}-${arch}.${ext}");
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "T3 Kanban (Alpha) 1.2.3 Installer",
          ...
          ...DMG_HIDDEN_FILE_LOCATIONS.map((location) => ({
            ...
            path:
              location.name === ".background.tiff"
                ? `apps/desktop/resources/dmg/dmg-background-latest.tiff`
      ...
      // entry advertises MimeType=x-scheme-handler/t3kanban; for OAuth deep links.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "T3 Kanban", schemes: ["t3kanban", "t3kanban-dev"] },
      ]);
        ...
        assert.deepStrictEqual(
          ...
          [
            ...
            [
              ...
              path.join(dmgDir, "dmg-background-nightly@2x.png"),
              "-out",
              path.join(dmgDir, "dmg-background-nightly.tiff"),
            ],
          ],
        );
      }),
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.t3tools.t3kanban</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.t3tools.t3kanban");
