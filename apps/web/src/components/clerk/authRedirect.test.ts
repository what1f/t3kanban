  ...
  it("removes a Clerk virtual pathname and callback params while preserving the desktop route", () => {
    expect(
      resolveClerkSignInProps(
        "t3kanban://app/CLERK-ROUTER/VIRTUAL/sign-up?__clerk_status=complete#/settings/connections",
        true,
      ),
    ).toEqual({
      forceRedirectUrl: "t3kanban://app/#/settings/connections",
      signUpForceRedirectUrl: "t3kanban://app/#/settings/connections",
    });
  });

  it("preserves a clean development desktop route", () => {
    expect(resolveClerkSignInProps("t3kanban-dev://app/#/settings/general", true)).toEqual({
      forceRedirectUrl: "t3kanban-dev://app/#/settings/general",
      signUpForceRedirectUrl: "t3kanban-dev://app/#/settings/general",
    });
  });
});
