...
describe("DesktopAppIdentity", () => {
  it.effect("uses the T3 Kanban userData path without probing T3 Code data", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        assert.equal(userDataPath, "/Users/alice/Library/Application Support/t3kanban");
      }),
    ),
  );
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["T3 Kanban (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "T3 Kanban (Alpha)");
