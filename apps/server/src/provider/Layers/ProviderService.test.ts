...
describe("agent browser access", () => {
        ...
        issueMcpCredential: (request) =>
          Effect.sync(() => {
            issued.push({
              ...
              capabilities: request.capabilities ?? new Set(),
            });
            return undefined;
          }),
  ...
  it.effect("issues task-only MCP access when agent browser access is off", () =>
    Effect.gen(function* () {
      const issued = yield* startSessionWith(false, asThreadId("thread-browser-off"));

      assert.deepEqual(
        issued.map((entry) => [...entry.capabilities]),
        [["task"]],
      );
    }).pipe(Effect.provide(NodeServices.layer)),
  );

  it.effect("issues task and preview MCP access when agent browser access is on", () =>
    Effect.gen(function* () {
  ...
  it.effect("keeps task access when the project disables browser access", () =>
    Effect.gen(function* () {
